...
export const COURSE_AMOUNT_PAISE = 50_000
export const COURSE_CURRENCY = 'INR'
export const COURSE_PRICE_LABEL = '₹500'
...
export async function confirmCapturedPayment(paymentId) {
  const { client } = getRazorpay()
  const payment = await client.payments.fetch(paymentId)
  const paid = payment.status === 'captured' || payment.status === 'authorized'
  const amountOk = Number(payment.amount) === COURSE_AMOUNT_PAISE
